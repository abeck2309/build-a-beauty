...
export default function SeasonBoard({ careerTeam, overall, position, archetype, buildRatings }: Props) {
  ...
  const [mobilePanel, setMobilePanel] = useState<"team" | "sim" | "gamecast" | "standings" | "history">("sim");

  useEffect(() => {
    document.querySelector<HTMLElement>(".season-dashboard")?.setAttribute("data-mobile-panel", mobilePanel);
  }, [mobilePanel]);
  ...
  return <>
    ...
    <nav className="mobile-panel-tabs" aria-label="Season sections"><button type="button" className={mobilePanel === "team" ? "is-active" : ""} onClick={() => setMobilePanel("team")}>Team</button><button type="button" className={mobilePanel === "sim" ? "is-active" : ""} onClick={() => setMobilePanel("sim")}>Sim</button><button type="button" className={mobilePanel === "gamecast" ? "is-active" : ""} onClick={() => setMobilePanel("gamecast")}>Game</button><button type="button" className={mobilePanel === "history" ? "is-active" : ""} onClick={() => setMobilePanel("history")}>Last 5</button><button type="button" className={mobilePanel === "standings" ? "is-active" : ""} onClick={() => setMobilePanel("standings")}>Standings</button></nav>
