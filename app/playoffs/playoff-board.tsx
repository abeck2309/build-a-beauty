...
export default function PlayoffBoard({ careerTeam, overall, position, archetype }: Props) {
  ...
  const { conference, playerSeries, westMatchups, eastMatchups } = useMemo(() => {
    ...
    const conferenceFinal = (semis: MatchupData[], side: "East" | "West"): MatchupData[] => side === playerConference ? [playerSeries] : [[seriesWinner(semis[0], 30), seriesWinner(semis[1], 31)]];
    const westMatchups: MatchupData[] = round === 0 ? westRound : round === 1 ? (playerConference === "West" ? westSemis.map((matchup, index) => index === playerSemiIndex ? playerSeries : matchup) : westSemis) : round === 2 ? conferenceFinal(westSemis, "West") : [[playerConference === "West" ? playerSeries[0] : playerConferenceWinner(westSemis, 50), playerConference === "East" ? playerSeries[0] : playerConferenceWinner(eastSemis, 60)] as MatchupData];
    const eastMatchups: MatchupData[] = round === 0 ? eastRound : round === 1 ? (playerConference === "East" ? eastSemis.map((matchup, index) => index === playerSemiIndex ? playerSeries : matchup) : eastSemis) : round === 2 ? conferenceFinal(eastSemis, "East") : [];
