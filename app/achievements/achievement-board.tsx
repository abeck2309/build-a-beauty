...
export default function AchievementBoard() {
  ...
  return <div className="achievements-page"><section className="achievement-hero"><div className="achievement-title"><p>Build a Beauty</p><h1>Achievements</h1><span>Track your Cups, awards, elite builds, records, and legacy milestones.</span></div><div className="achievement-actions"><a href="/">Home</a><a href="/profile">Profile</a><button type="button" onClick={() => void load()}>Refresh</button></div><div className={`achievement-notice ${signedIn ? "is-signed-in" : ""}`}>{signedIn ? <span>Your legacy is connected. Completed runs update progress automatically.</span> : <><span>You are not logged in. You can view the catalog, but unlock progress appears after login.</span><a href="/account?tab=login">Login</a></>}</div><div className="achievement-summary"><article><small>Unlocked</small><strong>{unlocked.length}/{achievements.length}</strong></article><article><small>Progress</small><strong>{Math.round((unlocked.length / achievements.length) * 100)}%</strong></article><article><small>Points</small><strong>{points}/{total}</strong></article><article><small>Remaining</small><strong>{achievements.length - unlocked.length}</strong></article></div><div className="achievement-progress"><span style={{ width: `${(unlocked.length / achievements.length) * 100}%` }} /></div><div className="achievement-filters"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search achievements..." /><select value={status} onChange={(event) => setStatus(event.target.value)}><option>All Status</option><option>Unlocked</option><option>Locked</option></select><select value={category} onChange={(event) => setCategory(event.target.value)}><option>All Categories</option>{["Modes", "Builds", "Awards", "Season", "Legacy"].map((item) => <option key={item}>{item}</option>)}</select><select value={rarity} onChange={(event) => setRarity(event.target.value)}><option>All Rarities</option>{["Common", "Rare", "Epic", "Legendary", "Special", "GOAT"].map((item) => <option key={item}>{item}</option>)}</select></div></section><p className="achievement-count">{loading ? "Loading achievements..." : `Showing ${filtered.length} achievements`}</p><section className="achievement-grid">{filtered.map((achievement) => <article key={achievement.id} className={`achievement-card ${achievement.unlocked ? "is-unlocked" : "is-locked"}`}><div className="achievement-icon" aria-hidden="true">{achievement.unlocked ? "★" : "▣"}</div><div className="achievement-copy"><div className="achievement-tags"><span className={`rarity-${achievement.rarity.toLowerCase()}`}>{achievement.rarity}</span><span>{achievement.category}</span></div><h2>{achievement.title}</h2><p>{achievement.description}</p></div><span className="achievement-status">{achievement.unlocked ? "Unlocked" : "Locked"}</span><footer><div><small>Reward</small><b>+{achievement.reward} pts</b></div><div><small>{achievement.unlocked ? "Complete" : "Locked"}</small><span>{achievement.unlocked ? "Achievement earned" : "Complete objective"}</span></div></footer></article>)}</section></div>;
}
